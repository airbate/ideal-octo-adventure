		...
		edge: {
			...
			translate: (path, data, func, abnormalFunc) => {
				...
				const mockTranslate = (text, fromLang, toLang) => {
					console.log('使用本地模拟翻译服务，从' + fromLang + '翻译到' + toLang);
					...
					for (let word in translationDict) {
						// 如果是从中文翻译成英文
						if ((fromLang.includes('chinese') || fromLang === 'zh') && (toLang === 'english' || toLang === 'en')) {
							translatedText = translatedText.replace(new RegExp(word, 'g'), translationDict[word]);
						// 如果是从英文翻译成中文
						} else if ((fromLang === 'english' || fromLang === 'en') && (toLang.includes('chinese') || toLang === 'zh')) {
							const reversedDict = {};
							for (let key in translationDict) {
								reversedDict[translationDict[key]] = key;
							}
							for (let word in reversedDict) {
								translatedText = translatedText.replace(new RegExp(word, 'g'), reversedDict[word]);
							}
						}
					}
					...
					if (translatedText === text) {
						if (toLang === 'english' || toLang === 'en') {
							return text + ' [Translated]';
						} else if (toLang.includes('chinese') || toLang === 'zh') {
							return text + ' [已翻译]';
						} else {
							return text + ' [Tr]';
						}
				...
				console.log("使用本地模拟翻译服务，跳过外部API调用");
				console.log("翻译参数: 从" + from + "到" + to + ", 文本数量: " + textArray.length);
				...
				for (var i = 0; i < textArray.length; i++) {
					var translatedText = mockTranslate(textArray[i], from, to);
					d.text.push(translatedText);
				}
